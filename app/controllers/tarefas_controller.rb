class TarefasController < ApplicationController
  ...
  def index
    @tarefas = Tarefa.all
    @comentarios_options_for_select = Comentario.all
    if params[:comentarios_id]
      @tarefas = Comentario.find(params[:comentarios_id]).includes(:comentarios).tarefas
  else
      @tarefa = Tarefa.all
  end
  end
  ...
  private
    ...
    def tarefa_params
      params.require(:tarefa).permit(:titulo, :conteudo, :usuario, :situação)
    end
end
